...
| A17 | Day 19–22 | 【阶段二】信号优雅降级（异常不崩、断网走本地兜底） | ✅ | 09-11：断网/LLM 失败这半边（`groq.ts` `callGroq()`：超时+网络异常+非2xx+JSON解析全部 catch，从不 throw，统一返回 `null`）在 A9 就已经做扎实了，`classifier.ts`/`starter-coach.ts` 消费 `null` 各自兜底。这次补的是"异常不崩"这半边一直没做的部分——`signals.ts`（真正的信号采集核心）全文原来零 try/catch，`emitSignalEvent()`（所有信号最终汇入的主干路径）包了一层，链路上任何一步意外失败不再是静默消失、没有痕迹的 unhandled rejection；`chrome.tabs.get`/`chrome.tabs.query` 两处真实存在的竞态（tab 在查询期间被关掉）加了 try/catch。`index.ts` 新增 `runSafely()` helper，把全部 fire-and-forget 的 `void (async () => {...})()` 消息处理/事件监听统一收口，出错至少打一条带消息类型的日志，不再是裸的 "Uncaught (in promise)"。chrome API 强相关代码，沿用仓库既定边界不补自动化测试，`npm run typecheck`/`npm test`（312/312）/`npm run build` 全过 |
...
| B10 | Day 11–14 | 【阶段二】自适应退让打磨 | 🔄 | 依赖 B2、J7。**契约v4 §3.7「冷却结束时丢弃冷却期之前的证据」落地**（`detector.ts` 的 `discardEvidenceFromBeforeCheckIn()`，`isDrifting`/`isStuck` 两条通道各在冷却闸门之后调一次）。没按契约原文做 `onCooldownEnd` 边缘检测，改成幂等的"证据起点早于 lastCheckInTs 就作废"——不用给 BState 加新字段、重复调用无副作用、`-Infinity` 初值天然不匹配。修的是真实场景：用户无视气泡不回答，冷却一结束旧证据立刻再次满窗口，5 分钟一次连环 check-in。新增 `cooldown.test.ts` 4 条，**有效性用"临时把修复删掉看断言是否变红"验证过**（2 条失败，恢复后全绿）。参数侧 Jay 09-01 按真机数据调过：黑名单命中新增 15s 快速通道、CREATOR 通用阈值 8min→5min。**继续调参需要真实使用数据，不是现在能做完的事** |
| B11 | Day 15–18 | 【阶段二】check-in / 微重启措辞反复调 | ✅ | 依赖 B7，demo 成败点之一。`wording.ts` 从"每种情况一句话"扩成变体池 + `pickVariant()` 轮换（不轮换的话一次会话答两次 check-in 会看到一模一样的回复）；新增 `stripSiteSuffix()` 和 `MAX_TITLE_LENGTH=42` 处理长标题撑坏气泡；补齐休息模式和收尾反思两组文案。测试沿用 B7 那套正则挡说教味词汇（should/stop/again/why），**每个变体都要过同一道闸**。★ **评估后否决「LLM 预生成 check-in 变体」**：会丢掉这层正则保证，而丢的正好是整个产品最要紧的那一刻——记下来免得下周有人再提一遍重新辩论 |
| B12 | Day 15–18 | 【阶段二】起步拆解 prompt 打磨 | 🔄 | 依赖 B6。**08-29 v1**：把「复述目标/伪装成计划/前置条件当动作/一串步骤」四类失败模式写成显式反例 + 12 词上限 + 显式英文；同时 `max_tokens` 200→1000（gpt-oss 的 reasoning 计入输出预算，真机上 reasoning 占了 182/200 导致 JSON 被截断）。**09-01 一天连改三版**：**v2**（按 Jay 方案）修 v1 教出来的编造问题——`review computer network for the exam` → `"flip to chapter 4."`，拆开「必须具体」和「不许编造」、换掉两条编号范例，另加 `hasFabricatedSpecific()` 正则守卫作第二道防线（12 条单测，`starter-coach.test.ts`）；**v3** 修 v2 那条退路本身的错——`pre study my new course` → `"Pick up your notes…"`，新课的笔记按定义不存在，真正的分界线不是「具体 vs 泛化」而是「需要已经拥有 vs 当场能造出来」，改成在规则前先声明认知边界、把可用对象正面圈定成两类；**v4** 修「合规但没用」——`"Open a new doc, type down data structure and algorithm."` 每条规则都过了却什么也没推进，根因是前三版全在优化"真不真实"、没有一条要求"有没有用"，补上「做完之后必须拥有或知道某样十秒前没有的东西」并换掉两条零信息量的 Good 范例。**★ 四版的直接原因全部是 few-shot 范例。** **v5（同日，思路换了）**：不再收紧措辞，改成**给模型补上它一直缺的那份信息**——起因是新建的评测集（`evals/`，`npm run eval:coach`）第一次跑分给出「通过率 12/12(100%) 但 SEARCH 形态占 83%」，暴露出四版约束叠加已把解空间挤到只剩"去搜索"一个点（`finish chapter 3 of the react docs` 被建议去搜索那份文档、私有课程录播被建议去公网搜）。决定性证据是那批里唯一不搜索的好答案恰是任务自带真实文件名的那条。v5 新增 `anchorContext`（当前 tab 的 title+url），从 `handleOnboardingSubmit` → `runStarterCoach` → `StarterCoachLLMCall` 透传，并配套防讨好指令+反例（无关页面必须完全忽略，否则是"编造换了个真实的锚"）。**同时建了评测基建**：16 条用例 / 12 条机械判据 / 形态分布分组 / 规则层 30 条离线单测。**v5 首跑成绩**：防讨好 2/2 全过（无关页面被彻底忽略），相关页面 1/2（3b1b 视频首次给出 `PLAY`——这个形态 v1~v4 从未出现过；react.dev 那条仍去搜索）。**v5.1（09-02）**补上失败那条的根因——规则 (a) 只要求"复用任务里的名字"、没要求"直接打开而不是去搜它"，同一个病在对照组里也有三条（react docs / transformer paper / 私有录播，最后一条**公网根本搜不到、执行必然失败**）；并修了指标本身（形态分布改按 none/use/ignore 三组分开，新增 `IGNORED_OPEN_PAGE`）——**指标设计错了比 prompt 写错更隐蔽，会让人把成功读成失败**。 没标 ✅ 是因为只修了幻觉/有用性这一类问题，archetype 推断等原本范围还没做；prompt 质量本身走真机手测，清单见 `docs/起步教练prompt-v0.md` §6 |
| B13 | Day 15–18 | 【阶段二】桌宠三态动画 | ✅ | 依赖 B4。**全部在 CSS 层做，不碰 Lottie**——素材是 7 秒单循环、8 个 marker 全空，`playSegments()` 无片段可切；`setSpeed()` 语义是反的（猫玩电线放慢读起来是"更困"不是"更警觉"）。observing = 上浮 3px + 暖色外发光，checkin = 一次性弹跳（跟徽章 `ap-badge-pop` 同拍 0.4s 同缓动，读起来是一个手势）。**09-02 定稿 observing 强度为四档对照里的 v2（强发光不变色，Jay 拍板）**：v1 真机反馈"太不明显"，v3/v4 变色档被否的原因不是不好看而是代价绕不开——猫身 hue≈191°、电线 hue≈345°，CSS filter 作用于整个 SVG，把猫转橙的同时电线会从粉红变青绿。四档对照保留在 devpreview 里当决策记录（v2 那格不加覆盖=线上效果）。checkin 刻意不加猫的动效：observing 只有 30px 徽章在变所以需要帮忙，checkin 时 220px 气泡整个弹出来不可能错过，**把"猫会变"这张牌留给最需要它的 observing，反差才立得住** |
| B14 | Day 15–18 | 【阶段二】check-in 交互 UI | ✅ | 依赖 B7。三个回答按钮 + 弱化的"Done for today"出口（契约要求随时可结束专注，但视觉上不能像第四个判定选项）、`role="status"`+`aria-live`、非 checkin 态按钮不进 DOM（否则键盘用户能 tab 到看不见的按钮上回车）、sticky 气泡不被下一帧顶掉、答完给一句 B7 反馈再摘回空白。**09-02 补齐最后三处**：① `:focus-visible` 焦点样式——08-29 刚修过"不该 tab 到隐藏按钮"，修完能 tab 到了却看不清落点，是半截工程；② `:active` 按下态——点击到气泡消失要绕 SW+storage 走一圈，那几十到两百毫秒里按钮毫无变化，用户以为没点上就会再点；③ **本地 `answered` 状态锁住整组按钮**，修的是真实的重复计数 bug（点两下 → `recordCheckInAnswer` 加两次让收尾统计虚高、`applyCheckInFeedback` 走两次让退让阶梯多跳一级）。原来那句"立刻 `pushMicroRestartToast` 摘掉 checkin 态"防的就是这个，**但那个"立刻"要绕 storage 走一圈，窗口只是变窄了没关上** |
| B15 | Day 19–22 | 【阶段二】收尾反思视图 + 角落专注时长显示（配角，别喧宾夺主） | ✅ | 依赖 J7。收尾视图（`SummaryPanel.tsx`+`summary.css`）：时长、用引号原样还给用户的任务声明、check-in/休息统计（零次的行直接不渲染——说"我一次都没打扰你"像邀功，说"未检测到走神"像报告系统状态），文案全走 `wording.ts`，语气是收工不是成绩单。**09-02 修了一个会在 demo 上翻车的 bug**：`saveStats` 原来只有 `recordCheckInAnswer`/`recordRestStart` 两个调用方，等于"一场专注只有在你走神过或休息过之后才存在起点"——专注 45 分钟一次没被打扰（**= 最理想的那条路径**）到结算时读不到统计，被现造一份 `startedTs=now`，收尾于是说 `"That was less than a minute of work."`。修法是起步教练完成时调 `startSessionStats()`。**角落专注时长**：`focusedMinutes` 这个 prop 声明了很久但真实侧边栏从没传过（**第四次"代码在、测试绿、功能是空的"，又是 devpreview 让它看起来是通的**），补上 `src/pet/useFocusedMinutes.ts`（30s tick，不自己走的话数字会卡住——显示过时的时长比不显示更糟，它在撒谎）。★ **按"什么会被扔掉"分层**：写 `startedTs`（SW）和算分钟数（`src/pet/`）在 B16 换悬浮桌宠时原样保留，只有 `main.tsx` 里读 storage 传 prop 那 5 行要重接 |
